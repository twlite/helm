export const buildAgentSystemPrompt = (args: {
  ...
  const blocks: string[] = [
    ...
    '- Prefer keyboard shortcuts when they are reliable for the current app and task, especially for common actions such as Ctrl+L for the browser address bar, Ctrl+F for find, Ctrl+S for save, Ctrl+A for select all, Tab or Shift+Tab for focus movement, Enter for submit/open, and Escape for dismiss.',
    '- Use shortcuts instead of mouse clicks when OCR or visual coordinate guessing would be less reliable. After using a shortcut, verify with a screenshot before claiming the result or continuing with dependent actions.',
    '- To click a UI element: move_mouse to its coordinates, capture_screenshot to confirm cursor is on target, then click_mouse.',
    '- To type in a field: prefer a known shortcut or Tab navigation to focus the field when available; otherwise click the field first, verify focus via screenshot, then type_text.',
    '- To open a URL in Firefox: use open_application to open/focus Firefox, screenshot to see it, press Ctrl+L to focus the address bar, type the URL, press Enter, screenshot to verify the page loaded.',
